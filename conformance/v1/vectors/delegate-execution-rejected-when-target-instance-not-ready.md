# Delegate Execution Rejected When Target Instance Not Ready
...
Delegation rejects a target `agent instance` that is not in `ready`.

## Preconditions

- one visible non-terminal parent `execution` exists
- one visible target `agent instance` exists in `paused` or `terminated`
...
- request success
- implicit resume or activation of the target `agent instance`

## Conformance Hooks Exercised

- target `agent instance` not in `ready` rejects `delegate execution`

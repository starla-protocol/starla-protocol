# Invoke Tool Rejected When Tool Deleted
...
One deleted tool rejects invocation.

## Preconditions

- one visible non-terminal `execution` exists
- one visible `tool definition` exists in `deleted`

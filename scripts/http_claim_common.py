...
def run_checks(runner: Runner, checks: list[tuple[str, Callable[[Runner], None]]]) -> dict[str, str]:
...
def render_report(
    ...
    lines = [
        f"# {implementation_name} {title} {run_date}",
        "",
        f"Implementation Claim: `{claim_path}`",
...
def check_agent_definition_list(runner: Runner) -> None:
    ...
    expect(
        any(item["agent_definition_id"] == "agent-def-enabled" for item in response.body),
        "enabled definition missing",
    )
...
def check_agent_instance_list(runner: Runner) -> None:
    ...
    expect_status(response, 200)
    expect(
        any(item["agent_instance_id"] == "agent-inst-primary" for item in response.body),
        "primary instance missing",
    )
...
def check_context_omits_absent_sections(runner: Runner) -> None:
    response = runner.post("/v1/agent-instances/agent-inst-primary/submit-work", {"input": {"message": "standalone"}})
    expect_status(response, 201)
    execution_id = response.body["execution_id"]
...
CORE_CHECKS: list[tuple[str, Callable[[Runner], None]]] = [
